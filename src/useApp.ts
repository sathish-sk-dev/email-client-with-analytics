...
import { toggleSelectAllMailItems } from "./features/mail-list/utils/mailListUtils";
...
export const useApp = (): UseAppHooks => {
  ...
  const fetchMailListCallback = useCallback(async () => {
    ...
    const mailList = toggleSelectAllMailItems(list, false, true);
    dispatch(setMailList({ selectedViewType: ViewType.INBOX, mailList }));
  }, [dispatch]);
